...
export type SiteData = { subjects: Subject[]; teachers: Teacher[]; gallery: GalleryItem[]; heroStats: HeroStat[]; testimonials: Testimonial[]; announcements: Announcement[]; contact: ContactDetails };
...
export const defaultSiteData: SiteData = {
  ...
  announcements: [
    { id: "open-batches", title: "New batches are open", description: "Book a free conversation for the 2024–25 term.", date: "2024-06-01", target: "All families" },
  ],
  contact: { address: "24, 12th Main\nIndiranagar, Bengaluru", phone: "+91 80 1234 5678", email: "hello@thestudyroom.in", hours: "Mon–Sat · 9:00 AM–7:00 PM" },
};
...
export function readSiteData(): SiteData {
  ...
  try {
    ...
    return { ...defaultSiteData, ...parsed, subjects: Array.isArray(parsed.subjects) ? parsed.subjects : defaultSiteData.subjects, teachers: Array.isArray(parsed.teachers) ? parsed.teachers : defaultSiteData.teachers, gallery: Array.isArray(parsed.gallery) ? parsed.gallery : defaultSiteData.gallery, heroStats: Array.isArray(parsed.heroStats) ? parsed.heroStats : defaultSiteData.heroStats, testimonials: Array.isArray(parsed.testimonials) ? parsed.testimonials : defaultSiteData.testimonials, announcements: Array.isArray(parsed.announcements) ? parsed.announcements : defaultSiteData.announcements, contact: parsed.contact || defaultSiteData.contact };
  } catch { return defaultSiteData; }
}
export function saveSiteData(data: SiteData) { if (typeof window !== "undefined") window.localStorage.setItem(STORAGE_KEY, JSON.stringify(data)); }
export function resetSiteData() { if (typeof window !== "undefined") { window.localStorage.removeItem(STORAGE_KEY); window.localStorage.removeItem("study-room-site-data-v1"); } }
