...
export default function Home() {
  ...
  const handleLogin = () => {
    toast("The student and parent portal is coming next.", {
      description: "We’re building a calmer way to stay close to learning.",
    });
  };
  ...
  return (
    <div className="site-shell">
      <header className="site-header">
        <div className="page-width header-inner">
          ...
          <nav className={`site-nav ${menuOpen ? "is-open" : ""}`} aria-label="Primary navigation">
            ...
            <button className="nav-login" type="button" onClick={handleLogin}>Portal login <ArrowUpRight size={15} /></button>
          </nav>
        </div>
      </header>
      ...
      <main id="top">
        ...
        <section className="content-section subjects-section" id="subjects">
          <div className="page-width">
            ...
            />
            <div className="subject-filters" aria-label="Find a class"><label className="subject-search"><span aria-hidden="true">⌕</span><input value={subjectQuery} onChange={(event) => setSubjectQuery(event.target.value)} placeholder="Search subjects, grades, or schedule" aria-label="Search subjects, grades, or schedule" /></label><div className="subject-selects"><select value={gradeFilter} onChange={(event) => setGradeFilter(event.target.value)} aria-label="Filter by grade level">{grades.map((grade) => <option key={grade}>{grade}</option>)}</select><select value={seatFilter} onChange={(event) => setSeatFilter(event.target.value)} aria-label="Filter by seat availability">{seatOptions.map((option) => <option key={option}>{option}</option>)}</select></div><div className="category-filters" role="group" aria-label="Filter subjects by category">{categories.map((category) => <button type="button" key={category} className={subjectCategory === category ? "is-active" : ""} onClick={() => setSubjectCategory(category)}>{category}</button>)}</div></div>
      ...
      <footer className="site-footer">
        <div className="page-width footer-inner"><span>© 2024 Rasi Maths Tuition Centre</span><span>Learning, thoughtfully.</span></div>
      </footer>
    </div>
  );
}
